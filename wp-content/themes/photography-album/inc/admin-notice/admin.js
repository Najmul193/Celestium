( function( $ ){
    $( document ).ready( function(){
      $( '.photography-album-btn-get-started' ).on( 'click', function( e ) {
          e.preventDefault();
          $( this ).html( 'Processing.. Please wait' ).addClass( 'updating-message' );
          $.post( photography_album_ajax_object.ajax_url, { 'action' : 'install_act_plugin' }, function( response ){
        ...
        $.ajax( ajaxurl,
          {
            type: 'POST',
            data: {
              action: 'photography_album_dismissed_notice_handler',
              type: type,
            }
          } );
      } );
